...
@click.command()
@click.option("--results-root", default="/data/data0/akane/kv-evict-bench", type=str, help="Root directory of all results")
@click.option("--output-json-filepath", default="/data/data0/akane/kv-evict-bench-collated.json", type=str, help="JSON savepath for all results")
...
def main(results_root, output_json_filepath, max_num):
    ...
    all_dirs = [results_root]
    for dirpath in all_dirs:
            unpruned_file = sorted(glob.glob(os.path.join(dirpath, "*none*.json")))
            # assert len(unpruned_file) == 1
            for filepath in unpruned_file:
                with open(filepath,  "r") as f:
                    model_name = filepath.split("/")[-1].split("-")[0]
                    results_dict[model_name] = dict()
                    unpruned_stats = json.load(f)
                    results_dict[model_name]["unpruned"] = unpruned_stats["rate"]
            ...
            for filepath in arith_mean_files:
                ...
                with open(filepath, "r") as f:
                    ...
                    results_dict[model_name][num]["arithmetic_mean"] = arith_mean_stats["rate"]
            ...
            for filepath in geo_mean_files:
                ...
                with open(filepath, "r") as f:
                    ...
                    results_dict[model_name][num]["geometric_mean"] = geo_mean_stats["rate"]
            ...
            for filepath in topk_files:
                ...
                with open(filepath, "r") as f:
                    ...
                    results_dict[model_name][num]["topk"][k] = topk_stats["rate"]
...
@click.command()
@click.option("--json-path", default="/data/data0/akane/kv-evict-bench-collated.json")
@click.option("--csv-path", default="/data/data0/akane/kv-evict-bench-collated.csv")
@click.option("--max-num", default=16, type=int, help="Max eviction number used for the sweep")
